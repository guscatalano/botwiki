...
export async function search(
  query,
  { limit = 10, tag, scanAll = false, count = true, client = null, expanded = false } = {}
) {
  ...
  // Announced before the search runs rather than after, so the count measures
  // queries people asked rather than queries that happened to succeed.
  // `client` is whatever the caller could observe about who is asking; the
  // store does not interpret it, it only passes it to subscribers.
  if (count) announce(searchHooks, { query: q, tag, client });
  ...
  // A thin result set is the one case where the corpus's own vocabulary is
  // worth borrowing. "the box keeps dropping off the network" shares no term
  // with a page that says "the igc0 trunk flaps", so no amount of ranking finds
  // it — but `network` and `igc0` appear together on this wiki's pages, and that
  // is a fact the corpus already knows.
  //
  // Only on thin results, deliberately. Expanding every query would let a dozen
  // borrowed words outvote the ones someone actually typed, which makes good
  // searches worse in order to rescue bad ones. Here the original results are
  // already ranked and keep their places; expansion only appends.
  if (!expanded && ranked.length < Math.min(3, limit)) {
    try {
      const assoc = await import('./assoc.js');
      const extra = await assoc.expand(terms, { limit: 6 });
      if (extra.length) {
        const seen = new Set(ranked.map((r) => r.slug));
        const more = await search(extra.map((e) => e.term).join(' '), {
          limit,
          tag,
          scanAll,
          count: false,
        ...
        for (const hit of more) {
          if (seen.has(hit.slug) || ranked.length >= limit) continue;
          seen.add(hit.slug);
          // Marked, because a result nobody's words asked for should say so.
          ranked.push({ ...hit, score: hit.score * 0.4, related: true });
        }
      }
    } catch {
      // Expansion is an improvement on failing to find anything. If it cannot
      // run, the thin result set was still the honest answer.
    }
  }
