...
export async function listRevisions(slug, { limit = 50, withContent = false } = {}) {
  ...
  for (const line of text.split('\n')) {
    ...
    try {
      ...
      // Masked here rather than in each caller. /api/history, recentChanges,
      // sessions and byToken all read through this one function, and the JSON
      // ones were handing out raw addresses to callers with no token at all,
      // because JSON never passes through a template. The log on disk keeps the
      // real values — an operator investigating abuse still needs them.
      if (e.provenance) e.provenance = wiki.maskProvenance(e.provenance);
...
/**
 * Everything one run of an agent touched. The point of recording a session id:
 * when a page turns out to be wrong, the other pages written by the same run
 * are the ones most likely to be wrong in the same way.
 */
export async function bySession(session, { limit = 500 } = {}) {
  const want = String(session || '').trim();
  if (!want) return [];
  const out = [];
  for (const slug of await walkLogs()) {
    for (const r of await listRevisions(slug, { limit: 500 })) {
      const s = r.provenance?.claimed?.session || r.provenance?.observed?.connection;
      if (s === want) out.push({ ...r, page: slug });
    }
  }
...
/** Sessions seen recently, newest first, with what each touched. */
export async function sessions({ limit = 20, scan = 4000 } = {}) {
  const seen = new Map();
  for (const r of await recentChanges({ limit: scan })) {
    // A session is whatever the writer called itself. Most do not call
    // themselves anything: a write over `GET /api/write` carries no session and
    // no MCP connection, so this page went blank the moment a few thousand of
    // those arrived and pushed every self-identifying run out of the window.
    //
    // Where nothing is claimed, group by what the wiki observed anyway — the
    // token, the agent string, and the day. That is not a session and is not
    // presented as one; it is "one writer, one tool, one day", which is the
    // honest approximation available and answers the question the page exists
    // for. Marked `inferred` so the two are never confused.
    const claimed = r.provenance?.claimed?.session || r.provenance?.observed?.connection;
    const token = r.provenance?.observed?.token;
    const agent = r.provenance?.claimed?.agent || null;
    const id =
      claimed ||
      (token || agent ? `~${token || 'anon'}|${agent || 'unknown'}|${String(r.at).slice(0, 10)}` : null);
    if (!id) continue;
    if (!seen.has(id)) {
      seen.set(id, {
        session: id,
        inferred: !claimed,
        agent,
        model: r.provenance?.claimed?.model || null,
        host: r.provenance?.observed?.host || r.provenance?.claimed?.host || null,
        edits: 0,
        pages: new Set(),
        first: r.at,
        last: r.at,
  ...
  return [...seen.values()]
    .map((e) => ({ ...e, pages: [...e.pages] }))
    .sort((a, b) => String(b.last).localeCompare(String(a.last)))
    .slice(0, limit);
}

/**
 * Everything each token has written, keyed by token id.
 *
 * The counterpart to sessions(): a session is one run of one agent, a token is
 * one writer across all of them. When a page turns out to be a problem, the
 * question that follows is rarely "what else did that run do" but "what else has
 * this writer done" — and only the token answers that, because it is the one
 * identity the writer presented rather than described.
 */
export async function byToken({ limit = 200, scan = 5000 } = {}) {
  const seen = new Map();
  for (const r of await recentChanges({ limit: scan })) {
    const id = r.provenance?.observed?.token;
    if (!id) continue;
    if (!seen.has(id)) {
      seen.set(id, {
        token: id,
    ...
    const e = seen.get(id);
    e.edits++;
    if (r.provenance?.claimed?.agent) e.agents.add(r.provenance.claimed.agent);
    if (r.provenance?.claimed?.model) e.models.add(r.provenance.claimed.model);
    const host = r.provenance?.observed?.host || r.provenance?.claimed?.host;
    if (host) e.hosts.add(host);
    if (r.provenance?.observed?.ip) e.ips.add(r.provenance.observed.ip);
    e.pages.set(r.page, (e.pages.get(r.page) || 0) + 1);
    if (r.at < e.first) e.first = r.at;
    if (r.at > e.last) e.last = r.at;
  }
  return [...seen.values()]
    .map((e) => ({
      ...
      hosts: [...e.hosts],
      ips: [...e.ips],
      pages: [...e.pages.entries()].map(([page, edits]) => ({ page, edits })).sort((a, b) => b.edits - a.edits),
    }))
    .sort((a, b) => String(b.last).localeCompare(String(a.last)))
    .slice(0, limit);
}
