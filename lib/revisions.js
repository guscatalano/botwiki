...
export async function sessions({ limit = 20 } = {}) {
  const seen = new Map();
  for (const r of await recentChanges({ limit: 1000 })) {
    const id = r.provenance?.claimed?.session || r.provenance?.observed?.connection;
    if (!id) continue;
    if (!seen.has(id)) {
      seen.set(id, {
        session: id,
        agent: r.provenance?.claimed?.agent || null,
