...
async function findByIndex(query, { limit, type, tag, minScore }) {
  ...
  for (const hit of await wiki.search(query, { limit: 50, tag, count: false })) lexical.set(hit.slug, hit.score);
...
export async function find(context, { limit = 5, type, tag, minScore = 0.02, scanAll = false, forceIndex = false } = {}) {
  ...
  // Describing what you want is searching. Counted here rather than in the
  // callers because find() has three of them and will have more.
  wiki.noteSearch({ query, tag, via: 'find' });
  ...
  const lexical = new Map();
  for (const hit of await wiki.search(query, { limit: 50, count: false })) lexical.set(hit.slug, hit.score);
  const maxLex = Math.max(1, ...lexical.values());
