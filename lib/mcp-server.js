...
export function createWikiServer({
  ...
  // Attachments, and the second condition that is not redundant even though it
  // looks it. files.ENABLED already refuses on a public instance, from
  // WIKI_PUBLIC. This adds the transport's own view of the same question, which
  // arrives by a different route: server/mcp.js may be told publicMode for a
  // connection for reasons the environment variable does not describe. Uploads
  // require both to say private. See lib/files.js for why this one is worth
  // saying twice.
  const filesOn = files.ENABLED && !publicMode;
  ...
  server.registerTool(
    ...
    async ({ query, limit, tag }) => {
      ...
      const hits = await wiki.search(query, {
        limit: limit ?? 10,
        tag,
        // Who asked, as the transport observed it — not as the caller claims.
        client: client ? `${client.name}${client.version ? ` ${client.version}` : ''}` : 'mcp',
      });
      if (!hits.length) {
  ...
  server.registerTool(
    ...
    async ({ page }) => {
      ...
      if (!doc) {
        const near = await wiki.search(page, { limit: 5, count: false });
        return fail(
          `No page named "${page}".` +
            (near.length
  ...
  // Attachments are absent, not merely refused, when the feature is off: an
  // agent should not be shown a tool it will always be told it cannot use.
  if (filesOn) {
    server.registerTool(
      'wiki_files',
      {
        ...
        description:
          'Every file stored on this wiki — images, PDFs, data — with its size, type and the URL a page references it by. Use it before uploading, to reuse what is already here rather than adding a second copy under a different name.',
        inputSchema: {},
      },
      async () => {
        const list = await files.listFiles();
        if (!list.length) return text('No files stored yet.');
        const { bytes } = await files.usage();
        return text(
          `${list.length} file(s), ${bytes} bytes total.\n\n` +
            list.map((f) => `${f.url}  ${f.mime}  ${f.size}B  ${f.modified}`).join('\n')
        );
      }
    );
  }
  ...
  if (!readOnly && filesOn) {
    server.registerTool(
      'wiki_upload',
      {
        title: 'Upload a file to the wiki',
        description:
          'Store an image or file and get back the URL and the markdown to embed it. The content is base64 of the raw bytes — send the file as it is, not a description of it. Uploading the same name again replaces it. Prefer a ```mermaid fence in the page over uploading a rendered diagram: a fence stays readable to whoever reads the markdown, and an image does not.',
        inputSchema: {
          name: z
            .string()
            .describe(
              'File name, optionally in folders, with a real extension — e.g. "diagrams/rack-layout.png". Lowercase letters, digits and dashes, like a page slug.'
            ),
          content: z.string().describe('The file, base64-encoded.'),
          model: z.string().optional().describe('Your model identifier, e.g. "claude-opus-5"'),
        },
      },
      async ({ name, content, model }) => {
        let buf;
        try {
          buf = Buffer.from(String(content || ''), 'base64');
        } catch {
          return fail('content must be base64.');
        }
        // Base64 decoding does not fail on junk, it just produces fewer bytes —
        // so a caller that sent a description instead of a file gets a confusing
        // "that does not look like a png" from the magic check rather than the
        // real problem. Catch the obvious case here where it can be named.
        if (!buf.length) return fail('content decoded to nothing. Send base64 of the file bytes.');
        try {
          const saved = await files.putFile(name, buf, { agent: model || 'mcp' });
          return text(
            `Stored ${saved.name} (${saved.size} bytes, ${saved.mime})${saved.replaced ? ', replacing what was there' : ''}.\n` +
              `URL: ${saved.url}\n` +
              `Embed it with: ${saved.markdown}\n\n` +
              `The file is not attached to any page until a page references that URL.`
          );
        } catch (err) {
    ...
    server.registerTool(
      'wiki_delete_file',
      {
        ...
        description:
          'Remove a stored file. Pages that still reference it will show a broken image, so check wiki_files for what uses it first.',
        inputSchema: { name: z.string().describe('File name as shown by wiki_files, e.g. "diagrams/rack-layout.png"') },
      },
      async ({ name }) => {
        try {
          const r = await files.deleteFile(name);
          return text(r.deleted ? `Deleted ${r.name}.` : `No file named ${r.name}.`);
        } catch (err) {
          return fail(err?.message || String(err));
        }
      }
