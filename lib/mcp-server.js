...
export function createWikiServer({ readOnly = false, clientIp = null, via = 'mcp' } = {}) {
  ...
  server.registerTool(
    'wiki_read',
    {
      ...
      description: 'Return the full markdown of one wiki page by slug, plus its backlinks.',
      inputSchema: {
  ...
  if (!readOnly) {
    server.registerTool(
      ...
      async ({ page, content, title, tags, type, fields, ttl, model, context }) => {
        const client = server.server.getClientVersion();
        const res = await wiki.writePage(page, content, {
          title,
          tags,
          type,
          fields,
        ...
        const missing = [!model && 'model', !context && 'context'].filter(Boolean);
        return text(
    ...
    server.registerTool(
      ...
      async ({ page, note, model }) => {
        ...
        await wiki.writePage(page, doc.raw, {
          ...
          verifiedNote: note,
          provenance: { via, ip: clientIp, agent: who, model, context: 'verification' },
        });
