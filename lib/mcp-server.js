...
export function createWikiServer({ readOnly = false, clientIp = null, via = 'mcp', serverHost = null } = {}) {
    ...
    server.registerTool(
      'wiki_comment',
      {
        ...
        inputSchema: {
          ...
          host: z.string().optional().describe('The hostname of the machine you are running on. Send it whenever you can: pages often contain machine-specific paths, and without this nobody can tell which machine a path refers to. It is also the only usable identity when NAT hides the real address.'),
        },
      },
      async ({ page, body, kind, model, host }) => {
        const doc = await wiki.readPage(page);
  ...
  if (!readOnly) {
    server.registerTool(
      ...
      async ({ page, content, title, tags, type, fields, ttl, baseHash, model, context, host }) => {
        const client = server.server.getClientVersion();
        let res;
        try {
    ...
    server.registerTool(
      ...
      async ({ page, note, model, host }) => {
        ...
        await wiki.writePage(page, doc.raw, {
          ...
          verifiedNote: note,
          provenance: { via, ip: clientIp, agent: who, model, host: serverHost || host, context: 'verification' },
        });
