...
// Bumped whenever the SHAPE or the CONTENT convention changes. Content counts:
// reindex() is incremental and skips pages whose files have not moved, so a
// change to how terms are produced does not reach already-indexed pages by
// itself. Only a version mismatch drops the table and rebuilds it from disk.
//
// 8: terms are stemmed.
// 9: trailing `._-` trimmed from tokens, so `content.` stopped being a
//    different term from `content`.
...
/**
 * Each page's strongest terms, for building the co-occurrence table.
 *
 * Ranked by stored term frequency and capped per page, because the association
 * build is quadratic in terms-per-page and a page's weakest terms carry the
 * least information about what it is about. Returns null when the index cannot
 * answer, so the caller can decline to expand rather than expand from nothing.
 */
export async function allPageTerms({ perPage = 40 } = {}) {
  const d = await open();
  if (!d) return null;
  try {
    // Frequency carried through, and more rows than the caller will keep.
    // Ranking by raw tf here would hand back each page's COMMONEST words, which
    // are the corpus's commonest words — the distinctive ones would be cut
    // before anything could weigh them. Only the caller knows the document
    // frequencies needed to judge that, so this returns a generous slice and
    // lets it decide.
    const rows = d
      .prepare(
        `SELECT slug, term, tf FROM (
           SELECT slug, term, tf, ROW_NUMBER() OVER (PARTITION BY slug ORDER BY tf DESC, term) AS rn
           FROM page_terms
         ) WHERE rn <= ?`
      )
      .all(Math.max(1, perPage));
    const bySlug = new Map();
    for (const r of rows) {
      let list = bySlug.get(r.slug);
      if (!list) bySlug.set(r.slug, (list = []));
      list.push({ term: r.term, tf: r.tf });
    }
    return [...bySlug];
  } catch (err) {
    lastError = err;
