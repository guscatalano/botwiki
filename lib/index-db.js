...
async function open() {
  ...
  try {
    ...
    if (found !== SCHEMA_VERSION) {
      for (const t of ['pages', 'page_tags', 'page_fields', 'page_terms', 'pages_fts']) db.exec(`DROP TABLE IF EXISTS ${t};`);
...
export async function remove(slug) {
  ...
  if (!d) return false;
  try {
    for (const t of ['pages', 'page_tags', 'page_fields', 'page_terms', 'pages_fts']) {
