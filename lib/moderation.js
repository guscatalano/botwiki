...
export async function report({ slug, reason, detail = '', ip = null, agent = null } = {}) {
  ...
  try {
    ...
    // Counted here rather than at the callers: the browser form, the GET form
    // and the MCP tool all reach this line, and so will anything added later.
    stats.record('report', { slug: entry.slug });
    return entry;
  } catch {
    return null;
  }
