...
const empty = () => ({
  ...
  clients: {},
  // route class -> array of counts, one per LATENCY_EDGES bucket. Still counts
  // and never events: a histogram says a route answered fourteen times between
  // 200ms and 500ms and cannot say which requests those were, who made them, or
  // when. Storing durations per request would be a request log by another name,
  // and this wiki promises it does not keep one.
  timing: {},
});

// Log-spaced, because latency problems are multiplicative: the gap between 20ms
// and 50ms matters as much as the gap between 2s and 5s, and a linear scale
// throws away the entire fast end where most requests live.
export const LATENCY_EDGES = [1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000];
const bucketFor = (ms) => {
  let i = 0;
  while (i < LATENCY_EDGES.length && ms >= LATENCY_EDGES[i]) i++;
  return i; // 0..LATENCY_EDGES.length, the last bucket being "over 5s"
};

// Deltas since the last flush, held per process.
let pending = { totals: {}, pages: {}, daily: {}, uniques: {}, clients: {}, timing: {} };
let timer = null;
let dirty = false;
 ...
 */
/**
 * How long a request took, in milliseconds, against a route CLASS.
 *
 * The class matters: timing `/w/lore/index` separately from `/w/soul/kern`
 * would give 217 buckets of one sample each and answer nothing. What a person
 * diagnosing a slow wiki needs is "reading any page is slow", which is a
 * statement about the handler.
 *
 * Recorded at the one place every request already passes through, for the
 * reason the rest of this file learned the hard way: a timer added per route is
 * a timer missing from every route somebody adds later.
 */
export function timed(routeClass, ms) {
  const cls = String(routeClass || 'other').slice(0, 64);
  const b = bucketFor(Number(ms) || 0);
  // Bucketed by day, because a cumulative histogram cannot tell "this route was
  // slow last week" from "this route is slow now" — and the first question
  // anybody asks after a fix is which of those they are looking at.
  const day = today();
  const forDay = (pending.timing[day] = pending.timing[day] || {});
  const row = (forDay[cls] = forDay[cls] || new Array(LATENCY_EDGES.length + 1).fill(0));
  row[b]++;
  dirty = true;
  schedule();
}
...
export function record(kind, { slug = null, n = 1, visitor = null, client = null } = {}) {
  ...
  // The live feed gets the kinds whose only funnel is this function. Writes,
  // searches and deletes are emitted from lib/live.js against the store hooks
  // instead, because those hooks carry more than a counter does — emitting them
  // here as well would double every one of them in the feed.
  if (kind !== 'write' && kind !== 'search') {
    live.emit(kind, { slug, detail: client ? { client: clientLabel(client) } : null });
  }
...
// Extracted so record() and timed() cannot drift on when they flush.
function schedule() {
  if (timer) return;
  timer = setTimeout(() => {
    timer = null;
  ...
  // Must never hold the process open: a wiki that will not shut down because it
  // owes someone a view count is worse than a lost view count.
  timer.unref?.();
}

// Subscribe to the store rather than asking every surface to remember us.
//
// These were counted in the MCP server and nowhere else, so the browser and the
// JSON API — where nearly all the traffic actually is — contributed nothing and
// the tiles read 0 indefinitely. Counting at the layer the data passes through
// means a surface written next year is counted without knowing this file exists.
wiki.onPageWritten(({ slug }) => record('write', { slug }));
wiki.onSearch(() => record('search'));
...
export async function flush() {
  ...
  const mine = pending;
  pending = { totals: {}, pages: {}, daily: {}, uniques: {}, clients: {}, timing: {} };
  dirty = false;
  ...
  return serialise(async () => {
    ...
    // Histograms add elementwise. Two processes serving the same wiki merge
    // cleanly, which a stored percentile would not — percentiles of percentiles
    // are meaningless, and that is the reason to keep the buckets rather than
    // the summary.
    state.timing = state.timing || {};
    for (const [day, classes] of Object.entries(mine.timing)) {
      const intoDay = (state.timing[day] = state.timing[day] || {});
      for (const [cls, row] of Object.entries(classes)) {
        const into = (intoDay[cls] = intoDay[cls] || new Array(LATENCY_EDGES.length + 1).fill(0));
        for (let i = 0; i < row.length; i++) into[i] = (into[i] || 0) + row[i];
      }
    }
    ...
    for (const day of Object.keys(state.timing || {})) if (day < cutoff) delete state.timing[day];
...
/**
 * Response times per route class, as approximate percentiles.
 *
 * Approximate is stated rather than hidden. A histogram can only say which
 * bucket a percentile falls in, so p95 comes back as the bucket's upper edge —
 * "at or under 500ms", never "487ms". A number with more digits than the method
 * supports is the thing to avoid here; see hindsight/counting-tokens.
 *
 * Sorted by p95 rather than by mean, because a route that is usually fast and
 * occasionally terrible is exactly what a mean hides and exactly what somebody
 * asking "why is the wiki slow" has noticed.
 */
export async function timings({ limit = 20, minRequests = 5, days = 1 } = {}) {
  const state = await snapshot();

  // Sum the requested window. Default is today, because the question this
  // exists to answer is "is it slow now" and a lifetime histogram answers
  // "was it ever", which after any fix is the same shape as never having fixed
  // it.
  const cutoff = new Date(Date.now() - (Math.max(1, days) - 1) * 86400000).toISOString().slice(0, 10);
  const merged = {};
  const seenDays = [];
  for (const [day, classes] of Object.entries(state.timing || {})) {
    if (day < cutoff) continue;
    seenDays.push(day);
    for (const [cls, row] of Object.entries(classes)) {
      const into = (merged[cls] = merged[cls] || new Array(LATENCY_EDGES.length + 1).fill(0));
      for (let i = 0; i < row.length; i++) into[i] = (into[i] || 0) + (row[i] || 0);
    }
  }

  const rows = [];
  for (const [route, row] of Object.entries(merged)) {
    const n = row.reduce((a, b) => a + b, 0);
    if (n < minRequests) continue;
    // The bucket a given percentile lands in, reported as that bucket's ceiling.
    const at = (q) => {
      let seen = 0;
      const want = q * n;
      for (let i = 0; i < row.length; i++) {
        seen += row[i];
        if (seen >= want) return LATENCY_EDGES[i] ?? null; // null = over the top edge
      }
      return null;
    };
    rows.push({
      ...
      p95: at(0.95),
      p99: at(0.99),
      slow: row.slice(bucketFor(1000)).reduce((a, b) => a + b, 0),
      buckets: row,
    });
  }
  // null p95 means "off the top of the scale", which sorts above everything.
  const key = (r) => (r.p95 === null ? Infinity : r.p95);
  rows.sort((a, b) => key(b) - key(a) || b.requests - a.requests);
  return {
    edges: LATENCY_EDGES,
    days: Math.max(1, days),
    covering: seenDays.sort(),
