...
export async function allOpen() {
  ...
  for (const [slug] of await openCounts()) {
    // Skip a discussion whose page is gone. Cleanup on delete handles this going
    // forward, but a queue is read by people acting on it, and an entry that
    // leads to a 404 wastes exactly the attention the queue exists to direct.
    // This also covers discussions orphaned before that cleanup existed.
    if (!(await wiki.readPage(slug, { includeHidden: true }))) continue;
...
/**
 * Drop a page's discussion when the page itself is deleted.
 *
 * Two things go wrong without this. A comment stays in the review queue pointing
 * at a page that 404s — and worse, a *new* page later written at the same slug
 * inherits a stranger's argument with something it has never seen.
 *
 * The page's own content is not lost by this: revisions keep it, and any abuse
 * report keeps its own record with its own detail. What goes is commentary that
 * no longer has anything to comment on.
 */
