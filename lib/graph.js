...
export async function buildGraph({
  ...
  const memoKey = `${corpusStamp(rows, 'graph')}|${minSimilarity}|${maxSimilarPerNode}|${includeSimilar}|${includeTags}|${minStrength}|${exactSimilarityMax}|${maxTagFanout}`;
  if (graphMemo.key === memoKey && graphMemo.graph) return graphMemo.graph;

  const opts = { minSimilarity, maxSimilarPerNode, includeSimilar, includeTags, minStrength, exactSimilarityMax, maxTagFanout };
  ...
  if (includeSimilar && N > 1) {
    ...
    const approx = N > exactSimilarityMax ? await similarByIndex(pages, { minSimilarity, maxSimilarPerNode }) : null;
